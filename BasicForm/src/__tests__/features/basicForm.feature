...
Scenario Outline: Valid field when leaving a textbox field
    ...
    Then the "<field>" field should show no error
    Examples:
        | field    | input     |
        | username | DR123     |
        | name     | DARIO     |
...
Scenario: Valid field when user inputs a username with 10 or less characters
    Given the user clicks the "username" field
    When the user types "PEPE321" inside the "username" field
    Then the "username" field should show no error
...
Scenario Outline: Valid field when user inputs uppercase characters
    Given the user clicks the "<field>" field
...
Scenario Outline: Valid field when user inputs name/surname not included un username
    Given the user clicks the "<field>" field
    And the user types "<input>" inside the "<field>" field
    When the user types "<input2>" inside the "<field2>" field
    Then the "<field2>" field should show no error
    Examples:
        | field    | input   | field2    | input2  |
        | username | DR123   | name      | DARIO   |
        | username | RD123   | surname   | RUA     |
        | name     | DARIO   | username  | DR123   |
        | surname  | RUA     | username  | RD123   |
...
Scenario Outline: Valid field when user input valid ID for the country
    Given the user clicks the "country" field
    And the user selects "<option>" option inside the list
    When the user types "<input>" inside the "id" field
    Then the "id" field should show no error
    Examples:
        | option    | input        |
        | SPAIN     | 12345678D    |
        | JAPAN     | 123456789012 |
...
Scenario Outline: Form submission - form completed and all data is valid
    Given the user types "<input>" inside the "username" field
    And the user types "<input2>" inside the "name" field
    And the user types "<input3>" inside the "surname" field
    And the user selects "<option>" option inside the list
